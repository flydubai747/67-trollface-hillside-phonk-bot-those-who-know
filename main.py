...
# Helper to remove previous AOP messages to prevent clutter
...
@bot.tree.command(name="aopnfhphnrnp", description="Set AOP to NF, HPH 402, and NRNP")
async def aopnfhphnrnp(interaction: discord.Interaction):
    ...
    await interaction.followup.send("AOP updated to NF, HPH 402, and NRNP.")

@bot.tree.command(name="aopnf", description="Set AOP to Northwind Falls")
async def aopnf(interaction: discord.Interaction):
    ...
    new_msg = await channel.send(embed=embed)
    save_msg_id(new_msg.id)
    await interaction.followup.send("AOP updated to Northwind Falls.")

@bot.tree.command(name="aopmw", description="Set AOP to Mapwide")
async def aopmw(interaction: discord.Interaction):
    ...
    new_msg = await channel.send(embed=embed)
    save_msg_id(new_msg.id)
    await interaction.followup.send("AOP updated to Mapwide.")
...
async def ssupoll(interaction: discord.Interaction, minutes: int, votes_needed: int):
    ...
    save_msg_id(msg.id) 
    await interaction.followup.send("Poll posted!")
